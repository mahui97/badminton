...
def render(image, pred, person_bbox, bbox_thre=0):
    ...
    :param pred: person joint list, n person * m joint * (x, y) = n * m * 2
    :param person_bbox: person bbox list, n person * (corner1, corner2, corner3, corner4, confidence_level)
    :param bbox_thre: bbox threshold
    :return: image after adding person bbox
    '''
    if pred is None:
        return image
    ...
    for person_pred in pred:
        ...
        for joint_pred in person_pred:
            cv2.circle(image, (int(joint_pred[0]), int(joint_pred[1])), i,
                       [0, 0, 255], 2)
            i = i + 1
...
def inference(detection_cfg,
    ...
    # # case for multi-process, we don't use it
    # else:
    ...
    if (len(all_result) == len(video_frames)) and (save_dir is not None):
        ...
        npy_path = os.path.join(save_dir, video_name.replace("mp4", "npy"))
        np.save(npy_path, all_result)
        ...
        #     prog_bar.update()
        # vwriter.release()
        print('\nVideo was saved to {}'.format(npy_path))
